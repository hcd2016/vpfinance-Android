 ...
 */
public class NewFundRecordsListFragment extends NewBaseFragment implements HttpDownloader.HttpDownloaderListener{
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View view = View.inflate(getActivity(), R.layout.new_fund_records_list_fragment, null);
        ...
        mListview.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (TextUtils.isEmpty(recordPoolId) || "0".equals(recordPoolId)) {
                    ...
                    Intent where = new Intent(getActivity(), FundRecordsDetailActivity.class);
                    where.putExtra(FundRecordsDetailActivity.UID, recordId);
                    startActivity(where);
                } else {
                    //定存宝
                    Intent where = new Intent(getActivity(), FundRecordsDepositDetailActivity.class);
    ...
    class MyAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (convertView == null) {
                ...
                convertView = View.inflate(getActivity(), R.layout.item_new_records_list, null);
                ...
                if (bean != null) {
                    ...
                    if (position != 0 && !TextUtils.isEmpty(month) && month.equals(mData.get(position - 1).getTenderMonth())) {
                        ...
                        if (mType.equals("1")) {//持有中
                            viewHolder.tvMonth.setText(month + "月出借");
                        } else {//已完成
                            viewHolder.tvMonth.setText(month + "月完成");
                        }
                    }
...
//                    }
                    ...
                    if ("true".equals(bean.getHaveRedPacket()) && (!TextUtils.isEmpty(bean.getShareUrl()))) {
                        ...
                        viewHolder.ll_share.setOnClickListener(new View.OnClickListener() {
                            ...
                            public void onClick(View v) {
                                final File imgFile = new File(getActivity().getCacheDir(), "redpack.png");
                                if (imgFile != null && imgFile.exists()) {
                                    //imgFile.delete();
                                } else {
                                    try {
                                        Utils.copyAssetsFileToPath(getActivity(), "redpack.png", imgFile.getAbsolutePath());
                                    } catch (IOException e) {
                                        e.printStackTrace();
                                    }
                                }
                                //								showShare("好友发来微品金融红包，个数有限先到先得，快去碰碰手气吧。", imgFile == null ? "" : imgFile.getAbsolutePath(), shareUrl);
                                SharePop sharePop = new SharePop(getActivity(), bean.getShareUrl(), imgFile == null ? "" : imgFile.getAbsolutePath(),
                                        "好友发来微品金融红包，个数有限先到先得，快去碰碰手气吧。", "微品金融拼手气红包");
                                sharePop.showAtLocation(mViewLl, Gravity.BOTTOM | Gravity.CENTER_HORIZONTAL, 0, ScreenUtil.getBottomStatusHeight(getActivity()));
                            }
                        });
                    }
                    ...
                    if (mType.equals("1")) {//持有中
                        viewHolder.tvDate.setText(split[0] + " 出借");//时间
                        viewHolder.tv_earnings_desc.setText("预计收益");
                    } else {//已完成
