...
public class NewPersonalInfoActivity extends BaseActivity implements View.OnClickListener {
    ...
    private TextView isBandActive;
    private LinearLayout ll_bindActive;
    ...
    private void initFind() {
        ...
        ll_bindActive = (LinearLayout) findViewById(R.id.ll_bindActive);
        isBandActive = (TextView) findViewById(R.id.isBandActive);
    ...
    protected void initView() {
        ...
        ll_bindActive.setOnClickListener(this);
        ll_bind_phone.setOnClickListener(this);

        SharedPreferencesHelper preferencesHelper = SharedPreferencesHelper.getInstance(NewPersonalInfoActivity.this);
                ...
                if (b) {
                    if (!close) {
                        if (!AppState.instance().logined()) {
                            startActivity(new Intent(NewPersonalInfoActivity.this, LoginActivity.class));
                            finish();
                            return;
                        }
                        startActivity(new Intent(NewPersonalInfoActivity.this, LockSetupActivity.class));

                        ArrayMap<String, String> map = new ArrayMap<String, String>();
                        map.put("switch", "on");
                        MobclickAgent.onEvent(NewPersonalInfoActivity.this, "GestureLock", map);
                    }
                    close = false;

                } else {
                    final SharedPreferencesHelper preferencesHelper = SharedPreferencesHelper.getInstance(NewPersonalInfoActivity.this);
                        ...
                        public boolean onTextConfrim(String value) {
                            if (value != null) {
                                ...
                                if (value.equals(pwd)) {
                                    ...
                                    Utils.Toast(NewPersonalInfoActivity.this, "密码错误!");
                                    close = true;
    ...
    public void onHttpSuccess(int reqId, JSONObject json) {
        ...
        if (reqId == ServiceCmd.CmdId.CMD_updateUserBasicInfo.ordinal()) {
            ...
            if ("1".equals(mUserInfoBean.isBindHxBank)){
                isBandActive.setText("已绑卡激活");
            }else{
                isBandActive.setText("未激活");
            }
...
//            ImageLoader.getInstance().displayImage(BaseUrl.mBaseUrl + userInfoBean.background , user_background);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.ll_bindActive:
                final Long userId = user.getUserId();
                if (TextUtils.isEmpty(user.getRealName())){
                    new AlertDialog.Builder(this).setMessage("请先实名认证并开通存管账户")
                            .setPositiveButton("去实名", new DialogInterface.OnClickListener() {
                                @Override
                                public void onClick(DialogInterface dialog, int which) {
                                    RealnameAuthActivity.goThis(NewPersonalInfoActivity.this);
                                }
                            })
                ...
                if (!"1".equals(mUserInfoBean.isOpen)){
                    new AlertDialog.Builder(this).setMessage("请先开通存管账户")
...
//                        application.guideConfig.put(FinanceApplication.SHOW_SETUP_GUIDE, false);
            ...
            case R.id.ll_bindemail:
                Intent intent1 = new Intent(NewPersonalInfoActivity.this, BindMailActivity2.class);
...
//                window.addFlags(WindowManager.LayoutParams.FLAG_DIM_BEHIND);
                ...
                menuWindow.setOnDismissListener(new PopupWindow.OnDismissListener() {
                    ...
                    public void onDismiss() {
                        ScreenUtil.lighton(NewPersonalInfoActivity.this);
                    }
                });
                break;
            case R.id.userHead:
    ...
    private void chargeUserName() {
        ...
        myDialogFragment.setOnTextConfrimListener(new MyDialogFragment.onTextConfrimListener() {
            ...
            public boolean onTextConfrim(String value) {
                if (value != null) {
                    if (VerifyUtils.checkUserName(NewPersonalInfoActivity.this, value)) {
                        newUserName = value;
                        mHttpService.updateUserBasicInfo(value, "" + user.getUserId());
                    } else {
                        Utils.Toast(NewPersonalInfoActivity.this, "用户名格式不正确");
                    }
                }
                return false;
            }
...
//                    getPhotoFromCamera();
                    CameraGalleryUtils.cameraAndClipByPermission(NewPersonalInfoActivity.this,CameraGalleryUtils.getSaveFile(HEADER_PHOTO));
                    break;
                // 相册选择图片
                case R.id.pickPhotoBtn:
//                    getPhotoFromPictureLibrary();
                    CameraGalleryUtils.galleryAndClipByPermission(NewPersonalInfoActivity.this,CameraGalleryUtils.getSaveFile(HEADER_PHOTO));
                    break;
                default:
                    break;
            }
...
//                        sp.putStringValue(SharedPreferencesHelper.USER_HEAD_URL + user.getUserId(), BaseUrl.mBaseUrl + imgUrl);
                        ...
                        mHandler.post(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(NewPersonalInfoActivity.this, finalStr, Toast.LENGTH_SHORT).show();
                            }
                        });
