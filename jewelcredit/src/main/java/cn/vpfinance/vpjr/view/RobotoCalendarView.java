...
public class RobotoCalendarView extends LinearLayout {
    ...
    private void setUpEventListeners() {
        ...
        tvToday.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                robotoCalendarListener.onTvTodayClick(currentCalendar);
            }
        });
    ...
    private void setUpMonthLayout() {
        ...
        dateTitle.setText(year+"年"+(month+1)+"月");
    }
    ...
    private void setUpDaysOfMonthLayout() {
        ...
        for (int i = 1; i < 43; i++) {
            ...
            dayOfTheMonthText.setTextColor(ContextCompat.getColor(context, R.color.text_666666));
            dayOfTheMonthContainer.setBackgroundResource(android.R.color.white);
            dayOfTheMonthContainer.setOnClickListener(null);
            dayOfTheMonthBackground.setBackgroundResource(android.R.color.white);
        }
    }
    ...
    private void markDayAsCurrentDay() {
        ...
        if (nowCalendar.get(Calendar.YEAR) == currentCalendar.get(Calendar.YEAR) && nowCalendar.get(Calendar.MONTH) == currentCalendar.get(Calendar.MONTH)) {
            ...
            markDayAsSelectedDay(nowCalendar);
    ...
    private void markDayAsSelectedDay(Calendar calendar) {
        ...
        if (circleImage2.getVisibility() == VISIBLE) {
            DrawableCompat.setTint(circleImage2.getDrawable(), ContextCompat.getColor(context, R.color.white));
        }
    }
    ...
    private void clearSelectedDay(Calendar calendar) {
        if (calendar != null) {
            ...
            TextView dayOfTheMonth = getDayOfMonthText(calendar);
            dayOfTheMonth.setTextColor(ContextCompat.getColor(context, R.color.text_666666));

            ImageView circleImage1 = getCircleImage1(calendar);
            ImageView circleImage2 = getCircleImage2(calendar);
            if (circleImage1.getVisibility() == VISIBLE) {
//                DrawableCompat.setTint(circleImage1.getDrawable(), ContextCompat.getColor(context, R.color.text_999999));
                boolean after = calendar.after(nowCalendar);
                if (after) {
                    circleImage1.setImageResource(R.drawable.red_point);
                    DrawableCompat.setTint(circleImage1.getDrawable(), ContextCompat.getColor(context, R.color.text_ff5757));
                } else {
                    circleImage1.setImageResource(R.drawable.gray_point);
                    DrawableCompat.setTint(circleImage1.getDrawable(), ContextCompat.getColor(context, R.color.text_999999));
                }
            }

            if (circleImage2.getVisibility() == VISIBLE) {
                DrawableCompat.setTint(circleImage2.getDrawable(), ContextCompat.getColor(context, R.color.text_999999));
            }
        }
    }
...
//        setUpWeekDaysLayout();
    ...
    public void markCircleImage1(Calendar calendar) {
        ...
        boolean after = calendar.after(instance);
        if (after) {
            circleImage1.setImageResource(R.drawable.red_point);
            DrawableCompat.setTint(circleImage1.getDrawable(), ContextCompat.getColor(context, R.color.text_ff5757));
        } else {
    ...
    public interface RobotoCalendarListener {
        ...
        void onRightButtonClick(Calendar currentCalendar);

        void onLeftButtonClick(Calendar currentCalendar);

        void onTvTodayClick(Calendar currentCalendar);
        void onCurrentMonthClick(Calendar currentCalendar);
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                if (Math.abs(moveX - downX) > Math.abs(moveY - downY) && (moveX - downX > 100)) {
                    currentCalendar.add(Calendar.MONTH, -1);
                    lastSelectedDayCalendar = null;
                    updateView();
                    robotoCalendarListener.onLeftButtonClick(currentCalendar);
                    return true;
                }else if (Math.abs(moveX - downX) > Math.abs(moveY - downY) && (downX - moveX > 100)) {
                    currentCalendar.add(Calendar.MONTH, 1);
                    lastSelectedDayCalendar = null;
                    updateView();
                    robotoCalendarListener.onRightButtonClick(currentCalendar);
...
//        return super.onTouchEvent(event);
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if(ev.getAction() == MotionEvent.ACTION_MOVE) {
            moveX = ev.getX();
            moveY = ev.getY();
            if (Math.abs(moveX - downX) > Math.abs(moveY - downY) && (Math.abs(moveX - downX) > 100)) {
                return true;
            }
        }
        return false;
